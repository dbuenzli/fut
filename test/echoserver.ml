...
let uerror (e, _, _) =
  let e = Bytes.unsafe_of_string (Unix.error_message e) in
  Futu.write Unix.stderr e 0 (Bytes.length e) >>= function
...
let echo s =
  let rec loop s buf =
    Fut.timeout 2. (Futu.read s buf 0 (String.length s)) >>= function
    | `Timeout ->
