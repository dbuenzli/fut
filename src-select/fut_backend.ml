...
module Unblock : sig
  ...
  let unblockc = Bytes.of_string "\x2A"
  let rec asap () =                                   (* write on !unblock_w *)
    try ignore (Unix.single_write !unblock_w unblockc 0 1) with
  ...
  let unblockbuf = Bytes.create 10
  let rec unblocked _ =                     (* consume data from !unblock_r *)
    try
      ignore (Unix.read !unblock_r unblockbuf 0 (Bytes.length unblockbuf));
